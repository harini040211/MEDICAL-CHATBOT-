...
def load_css():
    ...
    bg_image_base64 = ""
    image_path = "assets/wellness_bot_background.png"
    
    if os.path.exists(image_path):
        with open(image_path, "rb") as image_file:
            bg_image_base64 = base64.b64encode(image_file.read()).decode()
    
    # Use base64 image if available, otherwise use online fallback
    if bg_image_base64:
        background_css = f"background: url(data:image/png;base64,{bg_image_base64}) no-repeat center center fixed;"
    else:
        background_css = "background: url('https://images.unsplash.com/photo-1576091160550-2173dba999ef?w=1920&q=80') no-repeat center center fixed;"
    
    css_code = """
    <style>
    /* Main App Background - Wellness Bot Image */
    .stApp { 
        """ + background_css + """
    ...
    .metric-card h1, .metric-card h2, .metric-card h3, .metric-card p, .metric-card * {
    ...
    /* Make sure everything is highly visible */
    .stMarkdown {
        color: #000000 !important;
        font-size: 1.2rem !important;
        text-shadow: 1px 1px 2px rgba(255,255,255,0.8);
...
def show_admin_dashboard():
    ...
    with col1:
        ...
        if data['intent_dist']:
            intent_df = pd.DataFrame(data['intent_dist'], columns=['Intent', 'Count'])
    ...
    if data['daily_chats']:
        daily_df = pd.DataFrame(data['daily_chats'], columns=['Date', 'Chats'])
        daily_df['Date'] = pd.to_datetime(daily_df['Date'])
        
        fig = px.line(daily_df, x='Date', y='Chats',
                     title="Daily Chat Volume Trend",
                     line_shape='spline')
    ...
    if data['age_dist']:
        ...
        age_df = pd.DataFrame(data['age_dist'], columns=['Age Group', 'Count'])
...
def show_content_management():
    ...
    with tab2:
        ...
        try:
            ...
            if response_data:
                response_df = pd.DataFrame(response_data, columns=['Intent', 'Count'])
                fig = px.bar(response_df, x='Intent', y='Count',
                           title="Most Common Response Types")
                st.plotly_chart(fig, use_container_width=True)
            ...
            if feedback_data:
                st.subheader("📊 Response Feedback Analytics")
                feedback_df = pd.DataFrame(feedback_data, columns=['Feedback', 'Count'])
                
                col1, col2 = st.columns(2)
                with col1:
                    fig = px.pie(feedback_df, values='Count', names='Feedback',
                               title="User Response Ratings",
                               color_discrete_map={'thumbs_up': '#4CAF50', 'thumbs_down': '#F44336'})
                    st.plotly_chart(fig, use_container_width=True)
                ...
                with col2:
                    ...
                    st.metric("Thumbs Down", total_feedback - thumbs_up)
